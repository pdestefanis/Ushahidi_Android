...
public class Database {
	...
	public static final String[] INCIDENTS_COLUMNS = new String[] {
			...
			INCIDENT_LOC_LATITUDE, INCIDENT_LOC_LONGITUDE, INCIDENT_CATEGORIES,
	...
	public static final String[] CATEGORIES_COLUMNS = new String[] {
			...
			CATEGORY_IS_UNREAD, CATEGORY_POS };

	/**
	 * Columns of the table that stores off line incidents
	 */
	public static final String[] ADD_INCIDENTS_COLUMNS = new String[] {
			ADD_INCIDENT_ID, ADD_INCIDENT_TITLE, ADD_INCIDENT_DESC,
			ADD_INCIDENT_DATE, ADD_INCIDENT_HOUR, ADD_INCIDENT_MINUTE,
			ADD_INCIDENT_AMPM, ADD_INCIDENT_CATEGORIES, INCIDENT_LOC_NAME,
			INCIDENT_LOC_LATITUDE, INCIDENT_LOC_LONGITUDE, ADD_INCIDENT_PHOTO,
			ADD_INCIDENT_VIDEO, ADD_INCIDENT_NEWS, ADD_PERSON_FIRST,
			ADD_PERSON_LAST, ADD_PERSON_EMAIL };

	// Checkins messages
	public static final String[] CHECKINS_COLUMNS = new String[] { CHECKIN_ID,
			CHECKIN_USER_ID, CHECKIN_MESG, CHECKIN_DATE, CHECKIN_LOC_NAME,
			CHECKIN_LOC_LATITUDE, CHECKIN_LOC_LONGITUDE };

	// checkins users
	public static final String[] USERS_COLUMNS = new String[] { USER_ID,
	...
	public static final String[] DEPLOYMENT_COLUMNS = new String[] {
			DEPLOYMENT_ID, DEPLOYMENT_NAME, DEPLOYMENT_URL, DEPLOYMENT_DESC,
			DEPLOYMENT_CAT_ID, DEPLOYMENT_ACTIVE, DEPLOYMENT_LATITUDE,
	...
	private static final String INCIDENTS_TABLE_CREATE = "CREATE TABLE IF NOT EXISTS "
			...
			+ " TEXT, "
			+ INCIDENT_IMAGE
			+ " TEXT, " + INCIDENT_IS_UNREAD + " BOOLEAN NOT NULL " + ")";
	...
	private static final String ADD_INCIDENTS_TABLE_CREATE = "CREATE TABLE IF NOT EXISTS "
			...
			+ " TEXT NOT NULL, "
			+ ADD_INCIDENT_LOC_LATITUDE
			+ " TEXT NOT NULL, "
			+ ADD_INCIDENT_LOC_LONGITUDE
			+ " TEXT NOT NULL, "
			+ ADD_INCIDENT_PHOTO
			+ " TEXT, "
			+ ADD_INCIDENT_VIDEO
			...
			+ ADD_PERSON_FIRST
			+ " TEXT, "
			+ ADD_PERSON_LAST + " TEXT, " + ADD_PERSON_EMAIL + " TEXT " + ")";
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			db.execSQL("ALTER TABLE " + INCIDENTS_TABLE + " RENAME TO temp_"
					+ INCIDENTS_TABLE);
			db.execSQL(INCIDENTS_TABLE_CREATE);
			incidentsColumns
					.retainAll(Database.getColumns(db, INCIDENTS_TABLE));
			String cols = Database.join(incidentsColumns, ",");
			db.execSQL(String.format(
					"INSERT INTO %s (%s) SELECT %s FROM temp_%s",
					INCIDENTS_TABLE, cols, cols, INCIDENTS_TABLE));
			...
			db.execSQL("ALTER TABLE " + CATEGORIES_TABLE + " RENAME TO temp_"
					+ CATEGORIES_TABLE);
			db.execSQL(CATEGORIES_TABLE_CREATE);
			...
			db.execSQL("ALTER TABLE " + ADD_INCIDENTS_TABLE
					+ " RENAME TO temp_" + ADD_INCIDENTS_TABLE);
			db.execSQL(ADD_INCIDENTS_TABLE_CREATE);
			...
			db.execSQL(String.format(
					"INSERT INTO %s (%s) SELECT %s FROM temp_%s",
					ADD_INCIDENTS_TABLE, addIncidentCols, addIncidentCols,
					ADD_INCIDENTS_TABLE));
			...
			db.execSQL("ALTER TABLE " + CHECKINS_TABLE + " RENAME TO temp_"
					+ CHECKINS_TABLE);
			db.execSQL(CHECKINS_TABLE_CREATE);
			...
			db.execSQL(String.format(
					"INSERT INTO %s (%s) SELECT %s FROM temp_%s",
					CHECKINS_TABLE, checkinsCols, checkinsCols, CHECKINS_TABLE));
			...
			db.execSQL("ALTER TABLE " + CHECKINS_MEDIA_TABLE
					+ " RENAME TO temp_" + CHECKINS_MEDIA_TABLE);
			db.execSQL(CHECKINS_MEDIA_TABLE_CREATE);
			...
			db.execSQL(String.format(
					"INSERT INTO %s (%s) SELECT %s FROM temp_%s", USERS_TABLE,
	...
	public long createIncidents(IncidentsData incidents, boolean isUnread) {
		...
		initialValues.put(INCIDENT_LOC_LATITUDE,
				incidents.getIncidentLocLatitude());
		initialValues.put(INCIDENT_LOC_LONGITUDE,
				incidents.getIncidentLocLongitude());
		initialValues.put(INCIDENT_CATEGORIES,
				incidents.getIncidentCategories());
	...
	public long createAddIncident(AddIncidentData addIncident) {
		...
		initialValues.put(ADD_INCIDENT_CATEGORIES,
				addIncident.getIncidentCategories());
		initialValues.put(INCIDENT_LOC_NAME, addIncident.getIncidentLocName());
		initialValues.put(INCIDENT_LOC_LATITUDE,
				addIncident.getIncidentLocLatitude());
		initialValues.put(INCIDENT_LOC_LONGITUDE,
				addIncident.getIncidentLocLongitude());
		for (int i = 0; i < addIncident.getIncidentPhoto().size(); i++) {
			initialValues.put(ADD_INCIDENT_PHOTO, addIncident
					.getIncidentPhoto().get(i));
		}
	...
	public long createCheckinMedia(CheckinMedia checkinMedia) {
		...
		initialValues
				.put(MEDIA_THUMBNAIL_LINK, checkinMedia.getThumbnailLink());
	...
	public long createAddDeployment(String name, String url) {
		...
		initialValues.put(DEPLOYMENT_DATE, (new SimpleDateFormat(
	...
	public int addNewIncidentsAndCountUnread(
			ArrayList<IncidentsData> newIncidents) {
		addIncidents(newIncidents, true);
		return fetchUnreadCount();
	}

	public Cursor fetchAllIncidents() {
		return mDb.query(INCIDENTS_TABLE, INCIDENTS_COLUMNS, null, null, null,
				null, INCIDENT_DATE + " DESC");
	}

	public ArrayList<String> getAllIncidents() {
		ArrayList<String> IncidentDataCollection = new ArrayList<String>();
		Cursor IncidentCursor = fetchAllIncidents();
		boolean IsCurSet = true;
		IsCurSet = IncidentCursor.moveToFirst();
		while (IsCurSet) {
			if (IncidentCursor.getString(1) != "")
				IncidentDataCollection.add(IncidentCursor.getString(1));
			IsCurSet = IncidentCursor.moveToNext();
		}
		return IncidentDataCollection;
	}

	public Cursor fetchAllOfflineIncidents() {
		return mDb.query(ADD_INCIDENTS_TABLE, ADD_INCIDENTS_COLUMNS, null,
				null, null, null, ADD_INCIDENT_ID + " DESC");
	}

	public Cursor fetchAllCategories() {
		return mDb.query(CATEGORIES_TABLE, CATEGORIES_COLUMNS, null, null,
				null, null, CATEGORY_ID + " DESC");
	}

	public ArrayList<String> getAllCategories() {
		ArrayList<String> CategoriesDataCollection = new ArrayList<String>();
		Cursor CategoryCursor = fetchAllCategories();
		boolean IsCurSet = true;
		IsCurSet = CategoryCursor.moveToFirst();
		while (IsCurSet) {
			if (CategoryCursor.getString(2) != "")
				CategoriesDataCollection.add(CategoryCursor.getString(2));
			IsCurSet = CategoryCursor.moveToNext();
		}
	...
	public Cursor fetchIncidentsByCategories(String filter) {
		...
		String sql = "SELECT * FROM " + INCIDENTS_TABLE + " WHERE "
				+ INCIDENT_CATEGORIES + " LIKE ? ORDER BY " + INCIDENT_TITLE
				+ " COLLATE NOCASE";
	...
	public Cursor fetchIncidentsById(String id) {
		String sql = "SELECT * FROM " + INCIDENTS_TABLE + " WHERE "
				+ INCIDENT_ID + " = ? ORDER BY " + INCIDENT_TITLE
				+ " COLLATE NOCASE";
		return mDb.rawQuery(sql, new String[] { id });
	}

	public Cursor fetchAllCheckins() {
		return mDb.query(CHECKINS_TABLE, CHECKINS_COLUMNS, null, null, null,
				null, CHECKIN_DATE + " DESC");
	}
	...
	private Cursor query(String selection, String[] selectionArgs,
		...
		Cursor cursor = builder.query(mDbHelper.getReadableDatabase(), columns,
	...
	public Cursor fetchDeploymentById(String id) {
		...
		String[] columns = new String[] { BaseColumns._ID, DEPLOYMENT_NAME,
				DEPLOYMENT_LATITUDE, DEPLOYMENT_LONGITUDE, DEPLOYMENT_URL };

		return query(selection, selectionArgs, columns);
	}

	public Cursor fetchDeploymentByIdAndUrl(String id, String url) {
		String sql = "SELECT * FROM " + DEPLOYMENT_TABLE + " WHERE "
				+ DEPLOYMENT_ID + " = ? AND " + DEPLOYMENT_URL
				+ " =? ORDER BY " + DEPLOYMENT_NAME + " COLLATE NOCASE";
		return mDb.rawQuery(sql, new String[] { id, url });
	}

	public Cursor fetchDeploymentUrlById(String id) {
		String sql = "SELECT " + DEPLOYMENT_URL + " FROM " + DEPLOYMENT_TABLE
				+ " WHERE " + DEPLOYMENT_ID + " = ? ";
		return mDb.rawQuery(sql, new String[] { id });
	}

	public Cursor fetchCheckinsByUserdId(String id) {
		String sql = "SELECT * FROM " + CHECKINS_TABLE + " WHERE "
				+ CHECKIN_USER_ID + " = ? ORDER BY " + CHECKIN_ID
				+ " COLLATE NOCASE";
		return mDb.rawQuery(sql, new String[] { id });
	...
	public Cursor fetchCheckinsMediaByCheckinId(String id) {
		...
		String sql = "SELECT * FROM " + CHECKINS_MEDIA_TABLE + " WHERE "
				+ MEDIA_CHECKIN_ID + " = ? ORDER BY " + MEDIA_CHECKIN_ID
				+ " COLLATE NOCASE";
		return mDb.rawQuery(sql, new String[] { id });
	...
	public boolean deleteDeploymentByIdAndUrl(String id, String url) {
		String whereClause = "WHERE " + DEPLOYMENT_ID + " =? AND "
				+ DEPLOYMENT_URL + " =? ";
		String whereArgs[] = { id, url };
		return mDb.delete(DEPLOYMENT_TABLE, whereClause, whereArgs) > 0;
	}
	...
	public int fetchUnreadCount() {
		Cursor mCursor = mDb.rawQuery("SELECT COUNT(" + INCIDENT_ID + ") FROM "
				+ INCIDENTS_TABLE + " WHERE " + INCIDENT_IS_UNREAD + " = 1",
				null);
	...
	private int fetchUnreadCategoriesCount() {
		Cursor mCursor = mDb.rawQuery("SELECT COUNT(" + CATEGORY_ID + ") FROM "
				+ CATEGORIES_TABLE + " WHERE " + CATEGORY_IS_UNREAD + " = 1",
				null);
	...
	public void updateDeployment(String id) {
		String sql = "UPDATE " + DEPLOYMENT_TABLE + " SET " + DEPLOYMENT_ACTIVE
				+ "= ? WHERE " + DEPLOYMENT_ID + "= ?";
